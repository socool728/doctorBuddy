    ...
    <div class="col-lg-12 col-sm-12  col-xs-12 m-t-xl registration-wrap left no-pad">
        ...
        <div class="col-lg-12 col-sm-12 col-xs-12 no-pad">
            <div class="form-group item">    
                ...
                <input type="hidden" name="customer_id"  id="customer_id"  value="<?php echo isset($_SESSION['customer_id'])?$_SESSION['customer_id']:''?>" />
            </div>
        </div>
        
        <div class="col-lg-12 col-sm-12 col-xs-12 no-pad">
       <div id="usersection" <?php if(isset($_SESSION['customer_id']) && $_SESSION['customer_id']>0){?> class="hide" <?php } ?>>
        <div class="m-t-lg user_selection">
        ...
        <div class="row " id="newuser" >
            ...
            <div class="col-lg-6">
                <div class="form-group item">
                    <label  for="first-name">Email Address</label>
                    <input id="email" name="email" value="" title="Email Address"  class="form-control" type="email" required="required" <?php if(isset($_SESSION['customer_id']) && $_SESSION['customer_id']>0){?> disabled="disabled" <?php } ?>>  
                </div>  
            </div>
            <div class="col-lg-6">
                <div class="form-group item">
                    <label  for="first-name">Password</label>
                    <input id="password" name="password" value="" title="Password"  class="form-control" type="password" required="required" <?php if(isset($_SESSION['customer_id']) && $_SESSION['customer_id']>0){?> disabled="disabled" <?php } ?>>  
                </div>
                <div class="form-group item">
                    <label  for="first-name">Repeat Password</label>
                    <input id="password2" name="password2" value="" data-validate-linked="password" title="Repeat Password" class="form-control" type="password" required="required" <?php if(isset($_SESSION['customer_id']) && $_SESSION['customer_id']>0){?> disabled="disabled" <?php } ?>>  
                </div>                 
            </div>            
        ...
        <div class="col-lg-12 col-sm-12 col-xs-12 no-pad">
            ...
            <div class="col-lg-6 col-sm-6 col-xs-6 text-right">
                <button type="button" class="btn btn-red" role="button" onclick="return finish();">FINISH</button>
            </div>    
        </div>        
    </div>
