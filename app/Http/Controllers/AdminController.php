...
//            echo Session::get('admin_id')."KK";
    ...
    public function anyDelQuestion($id=''){       
        if($id!=''){
            ...
            Session::flash('flash_msg','Question deleted successfully !');
...
//                        $data[$detaild]['customer_name'] = $detail->customer_fname." ".$detail->customer_lname;
    ...
    public function anyAddQuestion(){
        ...
        if(isset($data['question'])){
            ...
            Session::flash('flash_msg','Question added successfully !');
            return Redirect::to('admin/questions');               
        }else{
...
//                }
            ...
            Session::flash('flash_msg','Question updated successfully !');
            return Redirect::to('admin/questions');               
        }else{
            $qndetail = '';
            $data['questions'] = DB::table('questions')->whereIn('question_type', array('radio','select','check'))->orderBy('display_order', 'ASC')->get();
...
//        $symptoms = DB::table('symptoms')->orderBy('symptom_id', 'DESC')->get();
    ...
    public function anyAddQuestiongroup(){
        ...
        if(isset($data['group_name'])){
            ...
            Session::flash('flash_msg','Question group added successfully !');
    ...
    public function anyEditQuestiongroup($id=null){
        ...
        if(isset($data['group_name'])){
            ...
            Session::flash('flash_msg','Question group updated successfully !');
            return Redirect::to('admin/questiongroup');               
        }else{
            $groupdetail = '';
            if($id >0){        
    ...
    public function anyDelQuestiongroup($id=''){       
       if($id!=''){
        ...
        Session::flash('flash_msg','Question group deleted successfully !');
   ...
   public function anyLanguage($type='',$id=''){
    ...
    public function anyCasefiles($type='',$id=''){
            ...
            switch ($type) {
                ...
                case 'counselor':
                        ...
                        // Get assigned case files of a particular counselor
                        $sql ="
                              SELECT CD.* FROM `casefile_to_counselor` AS CTH LEFT JOIN `customer_detail` AS CD 
                              ON  CTH.customer_detail_id = CD.customer_detail_id 
                              WHERE  CTH.counselor_id = $id AND CD.is_casefile =1 AND CD.customer_status =1 ORDER BY CD.`customer_detail_id` DESC"; 
                        break; 
                default:  
    ...
    public function anyKeyword($type='',$id=''){
        ...
        case  'add':
                        ...
                        if(!empty($post_arr) && $post_arr['form_submit'] == "save"){
                                ...
                                $validator = Validator::make(
                                    array(
                                        'keyword_name'  => $post_arr['keyword_name']
                                    ),
                                    array(
                                        'keyword_name' => 'required|unique:keywords,keyword_name'                                            
                                    )
                                );
                                ...
                                if ($validator->fails())
                                {
                                    ...
                                    $insert_data['keyword_name'] = $post_arr['keyword_name'];
                                    $insert_data['keyword_status'] = $post_arr['keyword_status'];
                                    
                                    if (count($insert_data) > 0 ) {
                                        DB::table('keywords')->insert($insert_data);
                                        Session::flash('flash_msg','Record added successfully');
                                    }
                                    return json_encode(array('error' => 0)); exit;
                            ...
                            return View::make('admin.addkeyword')->with(array('data' => $data));
                        }
        case  'edit': 
            
                        $keywordObj = DB::table('keywords')->where('keyword_id', $id)->get(); 
                        $data['keywordObj'] = $keywordObj[0];
                        ...
                        if(!empty($post_arr) && $post_arr['form_submit'] == "save"){
                                ...
                                $validator = Validator::make(
                                    ...
                                    array(
                                        'keyword_name' => 'required|unique:keywords,keyword_name,'.$id.',keyword_id',                                            
                                    )
                                );
                                ...
                                if ($validator->fails())
                                {
                                    ...
                                    $update_data['keyword_name'] = $post_arr['keyword_name'];
                                    $update_data['keyword_status'] = $post_arr['keyword_status'];
                                    if (count($update_data) > 0 ) {
                                        DB::table('keywords')->where('keyword_id', $id)->update($update_data);
                                        Session::flash('flash_msg','Record saved successfully');
                            ...
                            return View::make('admin.editkeyword')->with(array('data' => $data));
                        }
            
        case 'statuschange':
           if($id){
               
               $keyword = DB::table('keywords')->where('keyword_id','=',$id); 
               $oldStatus = $keyword->pluck('keyword_status');
               if($oldStatus)
                   $newStatus = 0;
               else
                    $newStatus = 1;
               $keyword->update(array('keyword_status'=>$newStatus));
               
               return  json_encode(array('error' => 0,'status' => $newStatus)); exit;
           } 
           return  json_encode(array('error' => 1,'err_msg'=>'Keyword is missing')); exit;
           
        case 'delete':
           if($id){
                DB::table('keywords')->where('keyword_id','=',$id)->delete();
                Session::flash('flash_msg','Record deleted successfully');
                return json_encode(array('error' => 0)); exit;
           } 
           return  json_encode(array('error' => 1,'err_msg'=>'Keyword is missing')); exit;  
        ...
        default: 
                ...
                return View::make('admin.keyword')->with(array('keywords' => $keywords,'data' => $data));
        }
...
//        Auth::logout();
    ...
    public function anyChecklogin(){
     ...
     public function anyAssigncounselor($id,$type=''){
        ...
        case 'assign': 
            ...
            // Assign counselor(s) to a particular case files
            $post_arr           = Input::all();               
            $customerDetailId =$id;
            
            /* Start:validation */
            $counselorArr = isset($post_arr['counselors'])?$post_arr['counselors']:array();
            if(count($counselorArr)==0){
                return  json_encode(array('error' => 1, 'err_msg' => " Please select or add atleast one Counselor")); exit;
}
            
            /* End:validation */
            
            
            $caseFileToCouns = DB::table('casefile_to_counselor')->where('customer_detail_id','=',$customerDetailId)->get();
            $alreadyAssignedCounIds = array();
            foreach($caseFileToCouns as $caseFileToCoun){
                if($caseFileToCoun->counselor_id != '')
                    $alreadyAssignedCounIds[] = $caseFileToCoun->counselor_id;
            }
            
            if(isset($post_arr['counselors']))
                $submitedCounIds = $post_arr['counselors'];
            else
                $submitedCounIds = array();

            $counselor_id = '';

            $CounIdsNeedToDelete = array_diff($alreadyAssignedCounIds,$submitedCounIds);            
            $CounIdsNeedToInsert = array_diff($submitedCounIds,$alreadyAssignedCounIds);

            
            if(count($CounIdsNeedToDelete)>0){
                foreach($CounIdsNeedToDelete as $CounId){
                    DB::table('casefile_to_counselor') 
                    ->where('counselor_id','=',$CounId)
                    ->where('customer_detail_id','=',$customerDetailId)        
                    ->delete();
                }
            ...
            if(count($CounIdsNeedToInsert)>0){
                foreach($CounIdsNeedToInsert as $CounId){
                    
                    $counObj =DB::table('counselors') ->where('counselors_id','=',$CounId)->first();
                    $email_id = $counObj->counselors_email_id;
                    ...
                    $insert_data['customer_detail_id'] = $customerDetailId;
                    $insert_data['counselor_id'] = $CounId;
                    $insert_data['date'] = date('Y-m-d H:i:s');       
                    $case_file_id = DB::table('casefile_to_counselor')->insertGetId($insert_data);
                    ...
                    $coun_name ="";
                    if($counObj->counselors_firstname)
                        $coun_name = $counObj->counselors_firstname;
                    
                    if($counObj->counselors_middlename)
                        $coun_name .= " ".$counObj->counselors_middlename;
                    
                    if($counObj->counselors_lastname)
                        $coun_name .= " ".$counObj->counselors_lastname;    
                    ...
                    $thingsToPassTemplate["site_link"] = URL::to('/'); 
                    $thingsToPassTemplate["site_start_link"] = URL::to('/')."/home/basic";      
                    $thingsToPassTemplate["receipent_name"] = $coun_name;  
                    $thingsToPassTemplate["link"] = $link; 
            ...
            Session::flash('flash_msg','Counselor(s) has been assigned successfully !');
            return json_encode(array('error' => 0)); exit;
        
        default:
            //Listing of counselors, for assignment of a case file
            $customerDetailId =$id;
            $sql = "SELECT * FROM counselors AS COUN LEFT JOIN country AS C
                    ON COUN.country_id= C.country_id WHERE COUN.counselors_status =1";
            $counObjs = DB::select($sql);
            $data['counObjs'] = $counObjs;
            
            $caseFileToCouns = DB::table('casefile_to_counselor')->where('customer_detail_id','=',$customerDetailId)->get();
            $alreadyAssignedCounsIds = array();
            foreach($caseFileToCouns as $caseFileToCoun){
                $alreadyAssignedCounsIds[] = $caseFileToCoun->counselor_id;
            }
            $data['alreadyAssignedCounsIds'] = $alreadyAssignedCounsIds;
            
            $caseFileObj = DB::table('customer_detail')->where('customer_detail_id','=',$customerDetailId)->first();
            $data['caseFileObj'] = $caseFileObj;            
            return View::make('admin.assigncounselor')->with(array('data' => $data));  
...
public function anyConversation($id='')
    {
        ...
        if(!$caseFileToHP){
            ...
            $hp_id=$caseFileToHP->healthcare_professional_id;
        }


        $data['casefile_to_healthcare_id']=$casefileToHealthcareId;
        $data['customer_detail_id']=$customerDetailId;
        $data['healthcare_professional_id']=$hp_id;
        $data['site_url']= URL::to('/'); 
        
        $conversations = DB::table('casefile_conversations')
         ->where('customer_detail_id', '=', $customerDetailId)
         ->where('healthcare_professional_id', '=', $hp_id)                
         ->orderby("date","DESC")
         ->get(); 
        $data['conversations'] = $conversations;
        
        return View::make('admin.conversation')->with(array('data'=>$data));
        exit;
        
    }
 public function anyReviews($action='',$id=''){ 
        $data   = array();
        $data['site_url'] = URL::to('/');
        switch($action)
        {
            case 'add': $post_arr           = Input::all();   
                        if(!empty($post_arr) && $post_arr['form_submit'] == "save")
                        {
                                // Check for mandatory fields not leaving as empty
                                $validator = Validator::make(
                                    array(
                                        'provider'  => $post_arr['healthcare_professional_id'],
                                        'score'  => $post_arr['review_score'],
                                        'customer' => $post_arr['customer_id']
                                    ),
                                    array(
                                        'provider' => 'required',
                                        'score' => 'required',
                                ...
                                {
                                    $insert_data['customer_id']= $post_arr['customer_id'];
                                    $insert_data['healthcare_professional_id'] = $post_arr['healthcare_professional_id'];
                                    $insert_data['review_score']  = $post_arr['review_score'];
                                    $insert_data['status'] = 1;
                                    $insert_data['comments']=$post_arr['comments'];
                                    $insert_data['admin_review'] = 1;
                                    $insert_data['date']=date('Y-m-d H:i:s');
                                   
                                    if (count($insert_data) > 0 ) {
                                       
                                        $review_info=DB::table('review')->where('review.customer_id', '=',$insert_data['customer_id'])
                                                                        ->where('healthcare_professional_id', '=',$post_arr['healthcare_professional_id'])
                                                                        ->where('status', '=',1)
                                                                        ->first();
                                        if(count($review_info)>0){
                                            DB::table('review')->where('review_id','=',$review_info->review_id)->update($insert_data);
                                            Session::flash('flash_msg','Review updated successfully');
                                        }else {
                                        DB::table('review')->insert($insert_data);
                                        Session::flash('flash_msg','Review added successfully');
                                    }
                                    }
                                    return json_encode(array('error' => 0)); exit;
                                } 
                        ...
                        { 
                            return View::make('admin.addreviews')->with(array('data' => $data));
                        }
                        break;
                    
            case 'edit':$data   = array();
                        $data['id']=$id;
                        $data['site_url'] = URL::to('/');
                        $reviewObj = DB::table('review')->where('review_id', $id)->first(); 
                        $data['review_score']=(!empty($reviewObj->review_score))? $reviewObj->review_score:'';
                        $data['healthcare_professional_id']=(!empty($reviewObj->healthcare_professional_id))? $reviewObj->healthcare_professional_id:'';
                        $data['comments']=(!empty($reviewObj->comments))?$reviewObj->comments:'';
                        $data['customer_id']=(!empty($reviewObj->customer_id))? $reviewObj->customer_id:'';
                        $post_arr           = Input::all();                     ;
                        ...
                        if(!empty($post_arr) && $post_arr['form_submit'] == "save")
                        {
                                ...
                                {
                                    $update_data['customer_id']= $post_arr['customer_id'];
                                    $update_data['healthcare_professional_id'] = $post_arr['healthcare_professional_id'];
                                    $update_data['review_score']  = $post_arr['review_score'];
                                    $update_data['status'] = 1;
                                    $update_data['comments']=$post_arr['comments'];
                                    $update_data['date']=date('Y-m-d H:i:s');
//                                    $update_data['admin_review'] = 1;
                                    if (count($update_data) > 0 ) 
                                    {
                                        DB::table('review')->where('review_id','=',$id)->update($update_data);
                                        Session::flash('flash_msg','Review updated successfully');
                                    }
                                    return json_encode(array('error' => 0)); exit;
                                } 
                        ...
                        { 
                            return View::make('admin.editreviews')->with(array('data' => $data));
                        }
                        break;
            ...
            case 'delete':if($id)
                            {
                                ...
                                Session::flash('flash_msg','Record deleted successfully');
                                return Redirect::action('AdminController@anyReviews');
                            } 
                            break; 
            default:
                    $reviewinfo = DB::table('review')->leftjoin('healthcare_professional', 'review.healthcare_professional_id', '=', 'healthcare_professional.healthcare_professional_id')
                            ->leftjoin('customers', 'review.customer_id', '=', 'customers.customer_id')
                            ->where('review.status', '=',1)
                            ->orderBy('review.date', 'DESC')->get();

                    if(count($reviewinfo)>0){
                       $data['reviewinfo'] =$reviewinfo;
                    }
                    return View::make('admin.reviews')->with(array('data' => $data));
                    exit;
        }
                  
    }
    
    public function anyProviderlist(){
        
        $data = Input::all();
        $result['provider_exist'] =false;
        ...
        if(isset($data['customer'])){  
            if($data['customer']>0){  
                $hp_arr=DB::table('casefile_to_healthcare')
                                ...
                                ->where('casefile_to_healthcare.customer_id', '=',$data['customer'])
                ...
                if(count($hp_arr)>0){ 
                    ...
                    foreach($hp_arr as $hp){
                        ...
                        $provider_arr[$hp->healthcare_professional_id] = $name;
                    }
                    $result['provider_arr'] =$provider_arr;
                }
    ...
    public function anyCheckreview(){
    
        $data = Input::all();
        $result['review_exist'] =false;
        $post_arr           = Input::all();   
        $customer_id=$post_arr['customer_id'];
        if((!empty($post_arr['healthcare_professional_id']))&&(!empty($customer_id))){  
            
                $review_info=DB::table('review')
                                ->where('review.customer_id', '=',$customer_id)
                                ->where('healthcare_professional_id', '=',$data['healthcare_professional_id'])
                                 ->where('status', '=',1)
                                ->first();
                if(count($review_info)>0){ 
                    $result['review_exist'] =true;
                    $result['comments']=$review_info->comments;
