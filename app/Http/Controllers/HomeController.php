...
//                                }
        ...
        if(!isset($_SESSION['customer_id']) && !isset($_SESSION['counselor_id']) && !isset($_SESSION['hp_id']) ) {  
        ...
        }else if(isset($_SESSION['counselor_id']) && $_SESSION['counselor_id'] !=''){
            
            $insert_data = array();
            $insert_data['customer_detail_id'] = $detail_id;
            $insert_data['customer_id'] = $customer_id;
            $insert_data['counselor_id'] = $_SESSION['counselor_id'];
...
public function anyPdfconversation(){    

        $post_arr           = Input::all(); 
        $customerDetailId = $post_arr['customer_detail_id'];
        $hp_id=$post_arr['hp_id'];
        $flag=$post_arr['flag'];
        $report = (new BaseController)->get_conversation_report($customerDetailId,$hp_id,$flag);
        $html =  View::make('template.conversation')->with(array('report' => $report))->render();
        //return View::make('template.conversation')->with(array('report' => $report));exit;

        $pdf = App::make('dompdf.wrapper');
        $pdf->loadHTML($html);
        $filename = $report['conversation_file_no'].'.pdf';
        $pdf->save(base_path().'/pdfreports/conversation_reports/'.$filename);
