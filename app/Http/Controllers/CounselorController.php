	...
	*/
    ...
    public function anyLogin($redirect=''){
        ...
        if($redirect == "casefileview"){
            $redirect_l = $redirect."?key=".$post['key'];
            $redirect=$redirect_l;
        }
...
//                )                      
    ...
    public function anyCasefiles(){
        
        if(!$this->anyChecklogin()){
              return Redirect::to('counselor/login/casefiles');   
        }         
       
        // Listing of casefiles those are assigned to or created by counselors
        $data = array();
        $data['site_url'] = URL::to('/');
        $caseFileObjs = DB::table('casefile_to_counselor')
                ->leftjoin('customer_detail','casefile_to_counselor.customer_detail_id', '=', 'customer_detail.customer_detail_id')
...
//                ->where("casefile_to_healthcare.healthcare_professional_email","=",$hpEmail)
                ->where("casefile_to_counselor.counselor_id","=",$_SESSION['counselor_id'])
                ->where("customer_detail.is_casefile","=",1)
                ->where("customer_detail.customer_status","=",1)
                ->groupby("casefile_to_counselor.customer_detail_id")
                ->orderby("casefile_to_counselor.casefile_to_counselor_id","DESC")
                ->get();
        $data['caseFileObjs'] = $caseFileObjs;
        return View::make('counselor.casefiles')->with(array('data'=>$data));
        exit;
    ...
    public function anyCasefileview(){
        ...
        if($this->anyChecklogin())
        {
            ...
            $casefileToCounselorId = $keyArr[1];
            
            
            $casefileToCounselor =DB::table('casefile_to_counselor')->where("casefile_to_counselor_id","=",$casefileToCounselorId)->first();
            
            if(!$casefileToCounselor){
                Session::flash('error_message',"No such record found in the system");
                return Redirect::to('counselor/dashboard');
            }else{
                $customerDetailId = $casefileToCounselor->customer_detail_id;
            }
            ...
            return View::make('counselor.view_casereport')->with(array('data'=>$data));
            exit;
            ...
            if(!$casefileToCounselor){
                ...
                $assignedCounselorId = (int)$casefileToCounselor->counselor_id;
                
                $assignedCounseloremail='';
                $counselorInfo =DB::table('counselors')->where("counselors_id","=",$assignedCounselorId)->first();
                if(!empty($counselorInfo))
                {
                    $assignedCounseloremail=$counselorInfo->counselors_email_id;
                }
            ...
            if($assignedCounseloremail!= ''){
                $assignedCounselorObj =$counselorInfo;
                if(is_object($assignedCounselorObj)){
                  $assignedCounselorId =$assignedCounselorObj->counselors_id ;
                }
            }
            
            if($assignedCounselorId){
                $redirect = "casefileview?key=$key";
                return Redirect::to('counselor/login/'.$redirect);
            }else{
                return Redirect::to('counselor/login');
            }            
