        ...
        $country = $state = array();
            ...
            if(!empty($post_arr)){
             /*   if (!preg_match('/[\'^£$%&*()}{@#~?><>,|=_+¬-]/', $post_arr['password']))
              ...
              if($counselorId){
                 $email = $post_arr['counselors_email_id']; 
                 $name = $post_arr['counselors_firstname'];
                 if(isset($post_arr['counselors_middlename']) && !empty($post_arr['counselors_middlename'])){
                     $name .= " ".$post_arr['counselors_middlename'];
                 }
                 
                 $name .= " ".$post_arr['counselors_lastname'];
                 
                // $this->anyVerifysend($counselorId,$email,$name);
                 
                 
                // Send Mail  
                $userName = $insert_arr['counselors_email_id'];
                $userPassword= trim($post_arr['password']);
                $email = $insert_arr['counselors_email_id'];
