	...
	*/    
    ...
    public function anyRegister(){
        ...
        $country = $state = array();
            ...
            $secret_key = "6LcVZLoZAAAAAAlW0lOWoV4ksXI7DKPqK3mNG3kf";
            $token = $post_arr['recap_token'];
            $ip_address = $_SERVER['REMOTE_ADDR'];
            
            $url = "https://www.google.com/recaptcha/api/siteverify?secret=".$secret_key."&response=".$token."&remoteip=".$ip_address;
            $request = file_get_contents($url);
            $response = json_decode($request);

            if(!$response->success){ 
                $err_msg  = '<br />';
                $err_msg .= "<i>Recaptcha : Some error occured!</i>";
...
//            )
            ...
            if($detail_id >0){
                ...
                $err_msg  = '<br />';
                $err_msg .= "<i>Something went wrong, please try again.!</i>";
                echo json_encode(array('error' => 1,'err_msg'=>$err_msg)); exit;
            }
...
//    }
    ...
    public function anyAssignedproviders($customerDetailId){
        ...
        $sql ="SELECT *
                ...
                LEFT JOIN `healthcare_professional` AS HP ON CH.healthcare_professional_id = HP.healthcare_professional_id
    ...
    public function anyConversations($casefileToHealthcareId){
        
        $casefileToHealthcareId = (int)$casefileToHealthcareId;
        
        // Login check
        if(!$this->anyChecklogin()){
            return Redirect::to("customer/login/conversations/$casefileToHealthcareId");
        }        
        
        $casefileToHpObj= DB::table('casefile_to_healthcare')->where('casefile_to_healthcare_id','=',$casefileToHealthcareId)->first();
        
        if(!is_object($casefileToHpObj) || count($casefileToHpObj) <=0){
             return Redirect::to('customer/dashboard');
        }
    ...
    public function anySendcomment(){
        ...
        $casefileToHealthcareId = $post_arr['casefile_to_healthcare_id'];
        $update_arr['display_status_for_healthcare'] =  6 ;//Waiting your Reply 
        $update_arr['display_status_for_customer'] =  5 ;//Replied 
        $casefileToHealthcareTable = DB::table('casefile_to_healthcare')->where('casefile_to_healthcare_id','=',$casefileToHealthcareId);
        $casefileToHealthcareTable->update($update_arr);
...
//        $link = asset('healthcare_professional/casefileview?key='.$key);
        ...
        $casefileHealthcareId =$casefileTOHelathcareObj->casefile_to_healthcare_id;
        $link = asset("healthcare_professional/conversation/$casefileHealthcareId");
...
public function anyReviews($action='',$id=''){ 
        ...
        switch($action)
        {
            case 'add': $post_arr           = Input::all();   
                        if(!empty($post_arr) && $post_arr['form_submit'] == "save")
                        {
                                ...
                                {
                                    $insert_data['customer_id']= $_SESSION['customer_id'];
                        ...
                        { 
                            return View::make('customer.addreviews')->with(array('data' => $data));
                        }
                        break;
            ...
            case 'edit':
                        ...
                        if(!empty($post_arr) && $post_arr['form_submit'] == "save")
                        {
                                ...
                                {
                                    $update_data['customer_id']= $_SESSION['customer_id'];
                        ...
                        { 
                            return View::make('customer.editreviews')->with(array('data' => $data));
                        }
                        break;
            ...
            case 'delete':if($id)
                            {
                                ...
                                Session::flash('flash_msg','Review deleted successfully');
                                return Redirect::action('CustomerController@anyReviews');
                            } 
                            break; 
            default:
                    if(!$this->anyChecklogin()){
                        return Redirect::to('customer/login/reviews');
                    }
                    ...
                    $reviewinfo = DB::table('review')->leftjoin('healthcare_professional', 'review.healthcare_professional_id', '=', 'healthcare_professional.healthcare_professional_id')
                            ->where('review.customer_id', '=',$_SESSION['customer_id'])
                            ->where('review.status', '=',1)
                            ->orderBy('review.date', 'DESC')->get();
                    if(count($reviewinfo)>0){
                       $data['reviewinfo'] =$reviewinfo;
                    }
                    return View::make('customer.reviews')->with(array('data' => $data));
                    exit;
        }
    ...
    public function anyCheckreview(){
        ...
        if((!empty($data['hp_id']))&&(!empty($customer_id))){  
            
                $review_info=DB::table('review')
                                ->where('review.customer_id', '=',$customer_id)
                                ->where('healthcare_professional_id', '=',$data['hp_id'])
                                 ->where('status', '=',1)
                                ->first();
                if(count($review_info)>0){ 
