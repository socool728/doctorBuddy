...
//            $mail->Send();
   ...
   public function get_casefile_report ($customerDetailId){
     ...
     public function send_mail ($templateLocation,$thingsToPassTemplate,$subject,$receipents,$attachments=array()){
         ...
         if(isset($thingsToPassTemplate['from_name']) && $thingsToPassTemplate['from_name'] !=''){
             $mailInputs["from_address_display"]= $thingsToPassTemplate['from_name'];
         }else{
             $mailInputs["from_address_display"]= \Config::get('constants.MAIL_FROM_ADDRESS_DISPLAY');
         }
         if(isset($thingsToPassTemplate['from_email']) && $thingsToPassTemplate['from_email'] !=''){
             $mailInputs["from_address_hidden"]= $thingsToPassTemplate['from_email'];
         }else{
...
public function get_conversation_report ($customerDetailId,$hp_id,$flag=''){
        ...
        $data['flag']=$flag;
        $data['site_url'] = URL::to('/');
        $data['conversation_file_no'] = 'DB'.$customerDetailId.'-'.$hp_id;
        if($customerDetailId >0 && $hp_id>0){      //ie particular case report
