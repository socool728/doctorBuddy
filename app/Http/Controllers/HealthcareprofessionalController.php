                ...
                if(isset($post['form_submit']) && $post['form_submit']=='save'){
                    ...
                    $validator = Validator::make(
                        array(
                            ...
                            'template_content'  => $post['template_text']
                        ),
                        array(
    ...
    public function anySavecasefilecomment(){
        ...
        if(isset($post_arr['form_submit']) && $post_arr['form_submit'] == 'save'){
            ...
            array(
                 ...
                 'treatment_plan'  => $post_arr['healthcare_comment'],
             ),
            array(
            ...
            if(isset($post_arr['casefile_to_healthcare_id']) && $post_arr['casefile_to_healthcare_id'] >0){
                ...
                $casefileToHealthcareObj = DB::table('casefile_to_healthcare')->where("casefile_to_healthcare_id","=",$casefileToHealthcareId);
                ...
                $update_arr['display_status_for_healthcare'] = 5 ;//Replied
                $update_arr['display_status_for_customer'] = 6 ;//Waiting your Reply        
...
//            }
            ...
            // conversation display link , for customer
             $link = asset("customer/conversations/$casefileToHealthcareId");
...
//            echo '<pre>';print_r($insuranceIds);die("yes");
            ...
            $hpTable->where("HP.healthcare_professional_status","=",1);
             ...
             $hpTable->select(
                    ...
                    'HPD.healthcare_professional_organization','HPD.healthcare_professional_fees','HPD.healthcare_professional_image',
                    'C.*'
                     );
             $hpTable->groupby('HP.healthcare_professional_id');
    ...
    public function anyAssignedcases(){
        ...
        $hpObj = DB::table('healthcare_professional')->where('healthcare_professional_id','=',$_SESSION['hp_id'])->first();
        $hpEmail=$hpObj->healthcare_professional_email_address;
    ...
    public function anySearchcases(){
        ...
        return View::make('healthcare_professional.searchcase')->with(array('data'=>$data));
        exit;
    ...
    public function anyAjaxsearchcaselisting(){
        ...
        $caseFileTable = DB::table('customer_detail AS CD')
        ->leftjoin('country AS C','CD.country_id', '=', 'C.country_id')            
        ->leftjoin('customer_symptoms AS CS','CD.customer_detail_id', '=', 'CS.detail_id');        
        

        if(!empty($post_arr) && $post_arr['form_submit'] == "search"){
  
            if(isset($post_arr['country']) && !empty($post_arr['country'])){
                $caseFileTable->Where('CD.country_id', '=', $post_arr['country']);
            }
            
            if(isset($post_arr['state_select']) && $post_arr['state_select'] !=''){
                $caseFileTable->Where('CD.customer_state','=',$post_arr['state_select']);
                
            }
            
            if(isset($post_arr['state_text']) && $post_arr['state_text'] !=''){
                $caseFileTable->Where('CD.customer_state', 'like', '%' . $post_arr['state_text'] . '%');               
            }

            if(isset($post_arr['city']) && $post_arr['city'] !=''){
                $caseFileTable->Where('CD.customer_city', 'like', '%' . $post_arr['city'] . '%');               
            }   
            
            if(isset($post_arr['sex']) && $post_arr['sex'] !=''){
                $caseFileTable->Where('CD.customer_sex', '=', $post_arr['sex']);               
            } 
            
            
            if(isset($post_arr['symptom']) && $post_arr['symptom'] !=''){
                $searchWords =  explode(",",$post_arr['symptom']);
                   
                   if(count($searchWords)>0){
                        $caseFileTable->where(
                            function($query)use ($searchWords){
                                foreach ($searchWords as $searchWord){
                                    $query->orWhere('CS.symptom_name', 'like', '%' . $searchWord . '%');
                                }
            ...
            if(isset($post_arr['keyword']) && $post_arr['keyword'] !=''){
                $searchWords =  explode(",",$post_arr['keyword']);
                   
                   if(count($searchWords)>0){
                        $caseFileTable->where(
                            function($query)use ($searchWords){
                                foreach ($searchWords as $searchWord){
                                    $query->orWhere('CD.known_disease', 'like', '%' . $searchWord . '%');
                                }
             ...
             $caseFileTable->where("CD.is_casefile","=",1);
             $caseFileTable->groupby('CD.customer_detail_id');
             
             $caseFileObjs = $caseFileTable->orderBy('CD.created_at','DESC')->get();       //print_r(DB::getQueryLog()); die();      
  
             $data['caseFileObjs'] = $caseFileObjs;
            
        }
        return View::make('healthcare_professional.ajaxsearchcaselisting')->with(array('data'=>$data));
        exit;
 
      
    }
    
    public function anySymptoms(){
        
        // create symptom tags for the searcch section in Search Case
        
        $post_arr = Input::all();
        $symptomArr = array();
        if(isset($post_arr['q']) && $post_arr['q'] !=''){
            
            $symptomObjs =  DB::table('symptoms')->Where('symptom_name', 'like', '%' . $post_arr['q'] . '%')
                            ->Where('status','=',1)
                            ->get(); 
            foreach($symptomObjs as $symptomObj){
                $symptomArr[] = $symptomObj->symptom_name;
            }
            
        }
        
        return json_encode($symptomArr); exit;
 ...
 // Listing,Add,Edit,Delete,View of Health care professional's templates
    public function anyReviews($action='',$id=''){
        ...
        if(!$this->anyChecklogin()){
            return Redirect::to('healthcare_professional/login/reviews');   
        } 
        ...
        switch($action){
            ...
            case "view":
                $reviewObj = DB::table('review')
                ->leftjoin('customers','review.customer_id','=','customers.customer_id')
                ->where("review.review_id","=",$id)
                ->first();     
                $data['customer_name']=$reviewObj->customer_fname." ".$reviewObj->customer_lname;
                $data['review_id']=$id;
                $data['comments']=$reviewObj->comments;
                $data['review_score']=$reviewObj->review_score;
                $data['date']=$reviewObj->date;
                return View::make('healthcare_professional.review_view')->with(array('data' => $data));
                break;
            default:
                $reviews = DB::table('review')
                ->leftjoin('customers','review.customer_id','=','customers.customer_id')
                ->where("review.healthcare_professional_id","=",$hpId)
                ->where("review.status","=",1)
                ->orderBy('review.date', 'DESC')->get();      
                
                $data['reviews'] = $reviews;
                return View::make('healthcare_professional.reviews')->with(array('hpObj'=>$hpObj,'data'=>$data));
                exit;   
        }
